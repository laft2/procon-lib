...
/**
 * @brief 最近共通祖先 (LCA: Lowest Common Ancestor) をダブリングにより求める構造体
 * 
 * 使用方法:
...
public:
    ...
    std::vector<std::vector<int>> parent;  // parent[k][v]: 頂点 v の 2^k 個上の親の頂点番号（存在しない場合は -1）
    ...
    int lca(int u, int v) const {
        ...
        // （LCA判定を愚直に二分探索すると O(log^2 N) かかるが、この方法により O(log N) でLCAの直前まで遡れる）
        for (int i = log_n - 1; i >= 0; --i) {
